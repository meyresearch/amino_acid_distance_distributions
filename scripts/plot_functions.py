...
def parse_command_line_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("-t", "--type", dest="data_type", type=str, choices=["pdb", "sim"],
                        help="data type for adjacency matrix")
    parser.add_argument("-f", "--file", dest="file", type=str,
                        help="PDB file or simulation matrix for adjacency matrix")
    parser.add_argument("-startd", "--start-dimensionality",
                        dest="start_dimensionality", type=float, help="starting value for dimensionality constant (A)")
    parser.add_argument("-endd", "--end-dimensionality", dest="end_dimensionality", type=float,
                        help="last value for dimensionality constant (A)")
    parser.add_argument("-starte", "--start-exponent", dest="start_exponent", type=int,
                        help="starting value for exponent (constant a)")
    parser.add_argument("-ende", "--end-exponent", dest="end_exponent", type=int,
                        help="starting value for exponent (constant a)")
    ...
    parser.add_argument("-rcsbd", "--rcsb-dimensionality", dest="d_rcsb", type=float,
                        help="constant A for rcsb algorithm in comparison plot")
    parser.add_argument("-alphad", "--alpha-dimensionality", dest="d_alpha", type=float,
                        help="constant A for alpha algorithm in comparison plot")
    parser.add_argument("-rcsbe", "--rcsbe-exponent", dest="e_rcsb", type=float,
                        help="constant a for rcsb algorithm in comparison plot")
    parser.add_argument("-alphae", "--alpha-exponent", dest="e_alpha", type=float,
                        help="constant a for alpha algorithm in comparison plot")
...
def check_required_arguments(argument_parser: argparse.ArgumentParser, given_algorithm: str, given_length: str,
    ...
    @return: None
    """
    if (given_algorithm == "rcsb" and starting_d is None) or (given_algorithm == "rcsb" and ending_d is None) or \
            (given_algorithm == "rcsb" and starting_e is None) or (given_algorithm == "rcsb" and ending_e is None) \
            or (given_algorithm == "rcsb" and given_length is None):
        argument_parser.error("rcsb requires -r, -startd, -endd, -starte, -ende")
    elif (given_algorithm == "alpha" and starting_d is None) or (given_algorithm == "alpha" and ending_d is None) or \
            (given_algorithm == "alpha" and starting_e is None) or (given_algorithm == "alpha" and ending_e is None) \
            or (given_algorithm == "alpha" and given_length is None):
        argument_parser.error("rcsb requires -r, -startd, -endd, -starte, -ende")
    elif (given_algorithm == "comp" and bootstrap_d is None) or (given_algorithm == "comp" and chunk_d is None) \
            or (given_algorithm == "comp" and bootstrap_e is None) or (given_algorithm == "comp" and chunk_e is None) \
            or (given_algorithm == "comp" and given_length is None):
        argument_parser.error("comp requires -r, -rcsbd, -alphad, -rcsbe, -alphae")
    elif given_algorithm == "bar" and given_length is not None:
        argument_parser.error("bar requires --range=None")
    elif (given_algorithm == "adj" and data_type is None) or (given_algorithm == "adj" and file is None) or \
            (given_algorithm == "adj" and given_length is not None):
        argument_parser.error("adj requires -t, -f")


"----------------------------------------------------------------------------------------------------------------------"

# algorithm = rcsb/alpha
"----------------------------------------------------------------------------------------------------------------------"
