...
def save_good_chain_lengths(confidence_dataframe: pd.DataFrame) -> None:
    """
...
def sort_ranges(pdb_files: list) -> None:
    """
    Sort RCSB PDBs into length ranges.
    @param: pdb_files: glob list of PDB files
    @return: None
    """
...
if algorithm == "alpha":
    ...
    save_good_chain_lengths(confidence_df)
elif algorithm == "rcsb":
    pdb_files = glob.glob("../data/rcsb/pdb_files/*.ent")
    sort_ranges(pdb_files)
