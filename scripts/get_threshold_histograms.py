...
def return_distance_histogram(log_file: str, given_algorithm: str, length_range: str, path_to_csvs: str, threshold: float) -> np.ndarray:
    ...
    if given_algorithm == "alpha":
        np.save(f"histogram_alpha_{length_range}_{threshold}.npy", histogram_array)
...
#return_distance_histogram(log_file="log.txt", given_algorithm=algorithm, length_range=str{length}, path_to_csvs=f"../data/rcsb/secondary_structures_{length}.csv")
